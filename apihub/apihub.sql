...
as $$
  insert into basic_auth.users (email, pass, role) values
    (signup.email, signup.pass, 'apiuser');
$$ language sql;

-------------------------------------------------------------------------------
-- User management (!!!! changed)

create or replace view apiusers as
select
...
posts (
  ...
  title      text not null,
  body       text not null,
  apiuser     text not null references basic_auth.users (email)
...
grant select on table pg_authid, basic_auth.users, posts, comments, apiusers to anon;
...
--grant apiuser to anon;
grant anon to authenticator;
grant apiuser to authenticator;    

grant select, insert, update, delete
  on basic_auth.tokens, basic_auth.users to anon, apiuser;
grant select, insert, update, delete
  on table posts, comments to apiuser;
grant select on table apiusers to apiuser;

grant usage, select on sequence posts_id_seq, comments_id_seq to apiuser;

grant usage on schema public, basic_auth to anon, apiuser;

ALTER TABLE posts ENABLE ROW LEVEL SECURITY;
drop policy if exists apiusers_eigenedit on posts;
create policy apiusers_eigenedit on posts
  using (true)
  with check (
    apiuser = basic_auth.current_email()
  );

ALTER TABLE comments ENABLE ROW LEVEL SECURITY;
drop policy if exists apiusers_eigenedit on comments;
create policy apiusers_eigenedit on comments
  using (true)
  with check (
    apiuser = basic_auth.current_email()
  );
...
------- Data
insert into basic_auth.users (email,pass,name,role,verified,appname) values
('joe@begriffs.com','nelson', 'Joe Nelson', 'apiuser','TRUE','appkey1'),
('ruslan.talpa@gmail.com','talpa', 'Ruslan Talpa', 'apiuser','TRUE','appkey1');



insert into posts (title,apiuser,body) values
...
insert into comments (body, apiuser, post) values
